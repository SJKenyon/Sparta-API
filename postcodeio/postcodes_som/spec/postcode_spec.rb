...
describe Postcodesio do
  ...
  context 'requesting information on a single postcode works correctly' do
    ...
    before(:all) do
      random_postcode = RandomPostcodeService.new.get_random_postcode
      @single_service = Postcodesio.new.single_postcode_service
      @single_service.get_single_postcode(random_postcode)
    end

    it "should respond with a status message of 200" do
      expect(@single_service.get_response).to eq 200
    end

    it "should have a results hash" do
      expect(@single_service.print_results).to be_kind_of(Hash)
    end

    it "should return a postcode between 5-7 in length"  do
      expect(@single_service.get_length).to be_between(5,7)
    end

    it "should return an quality key integer between 1-9" do
      expect(@single_service.quality_key).to be_kind_of(Integer)
      expect(@single_service.quality_key).to be_between(1,9)
    end

    it "should return an ordnance survey eastings value as integer" do
      expect(@single_service.eastings).to be_kind_of(Integer)
    end
    ...
    it "should return a country which is one of the four constituent countries of the UK" do
      expect(@single_service.country).to match("England").or match("Scotland").or match("Wales"). or match("Northern Ireland")
    end

    it "should return a string value for NHS authority " do
      expect(@single_service.nhs).to be_kind_of(String)
    end

    it "should return a longitude float value" do
      expect(@single_service.longitude).to be_kind_of(Float)
    end

    it "should return a latitude float value" do
      expect(@single_service.latitude).to be_kind_of(Float)
    end

    it "should return a parliamentary constituency string" do
      expect(@single_service.parliment).to be_kind_of(String)
    end

    it "should return a european_electoral_region string" do
      expect(@single_service.eu_elect).to be_kind_of(String)
    end

    it "should return a primary_care_trust string" do
      expect(@single_service.primary).to be_kind_of(String)
    end

    it "should return a region string" do
      expect(@single_service.region).to be_kind_of(String).or be nil
    end

    it "should return a parish string" do
      expect(@single_service.parish).to be_kind_of(String).or be nil
    end

    it "should return a lsoa string" do
      expect(@single_service.lsoa).to be_kind_of(String)
    end
